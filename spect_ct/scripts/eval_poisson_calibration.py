...
from spect_ct.pipeline.inference import denoise_views, load_basicsr_net
...
def main() -> None:
    ...
    # 提示：请在仓库根目录运行（确保能 import basicsr/spect_ct）
    try:
    ...
    try:
        ...
        if str(e).endswith("No module named 'cv2'") or "cv2" in str(e):
            raise ModuleNotFoundError(
                "缺少依赖 cv2（opencv-python）。请在你的训练环境里运行，例如：\n"
                "  conda run -n KAIR python spect_ct/scripts/eval_poisson_fit.py ...\n"
            ) from e
        raise
