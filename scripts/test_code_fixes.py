...
def test_dbsnl():
    ...
    from basicsr.archs.dbsnl_arch import DBSNl, DBSNlLight, CentralMaskedConv2d

    # Test CentralMaskedConv2d
    print('Testing CentralMaskedConv2d...')
    conv = CentralMaskedConv2d(64, 64, kernel_size=3, padding=1)
    x = torch.randn(1, 64, 16, 16)
    y = conv(x)
    # Check that center weight is masked
    mask = conv.mask
    assert mask[0, 0, 1, 1] == 0, "Center weight should be masked"
    print(f'  ✓ Center weight is properly masked')
...
def test_fpn_fusion():
    ...
    from basicsr.archs.unet_fpn_fusion_arch import UNetFPNFusion, UNetFPNFusionLight
...
def test_poisson_loss():
    ...
    from basicsr.losses.poisson_loss import PoissonNLLLoss, PoissonNLLLossSimple
...
def test_n2v_model():
    ...
    from basicsr.models.noise2void_model import Noise2VoidModel, Noise2VoidBlindSpotModel
...
def test_n2b_model():
    ...
    from basicsr.models.neighbor2neighbor_model import Neighbor2NeighborModel
...
def test_gpu(device='cuda'):
    ...
    from basicsr.archs.dbsnl_arch import DBSNl
    from basicsr.archs.unet_fpn_fusion_arch import UNetFPNFusion
