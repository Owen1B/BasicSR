...
from prime.pipeline.experiment import (
    find_config_for_experiment,
    list_experiments,
    parse_patient_indices,
    resolve_patients,
)
from prime.pipeline.io import load_projection_i16, load_recon_f32, save_projection_i16_round_clip
from prime.pipeline.metrics import PoissonBinStats, lpips_views_mean, psnr_ssim_recon
from prime.pipeline.original_recon_cache import ensure_original_recon, resolve_patient_paths
...
def _load_toolbox_defaults() -> dict:
    """Load interactive defaults from `scripts/toolbox_defaults.json` if present."""
...
def run_experiment(
    *,
    ...
    try:
        from prime.pipeline.threeproj3recon import process_patient_3proj3recon  # noqa: F401
    except ModuleNotFoundError as e:
        if "matplotlib" in str(e):
            raise ModuleNotFoundError(
                "缺少依赖 matplotlib（用于 GIF/可视化）。请在当前 Python 环境安装：\n"
    ...
    for pi, patient in enumerate(patients):
        ...
        if use_synth_thin:
            ...
            for k in thin_factors:
                ...
                xk = _poisson_thin_binomial(y20i, k=kk, rng=rng_k).astype(np.float32, copy=False)
                tag = f"x{kk:g}"
        ...
        if rc.compute_metrics:
            try:
                from prime.pipeline.io import load_projection_f32
                # PSNR/SSIM (range-aware): prefer BasicSR metrics (cv2-based) when available; fallback otherwise.
                try:
                    from basicsr.metrics import calculate_psnr_range, calculate_ssim_range  # type: ignore
                    _use_basicsr = True
                except Exception:
                    _use_basicsr = False
                    from prime.pipeline.simple_metrics import psnr_range, ssim_range
...
def _cmd_run(args: argparse.Namespace) -> None:
    ...
    ckpt = Path(args.ckpt) if args.ckpt else _pick_ckpt(exp_dir)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_run.add_argument("--exp-name", type=str, default=None, help="output name under outputs/")
    add_common(p_run)
    ...
    p_batch.add_argument("--match", type=str, default=None, help="fnmatch pattern for experiment names, e.g. converged_*")
    ...
    def _cmd_recon_results(args: argparse.Namespace) -> None:
        ...
        from prime.pipeline.original_recon_cache import resolve_patient_paths
    ...
    p_recon.add_argument("--results-dir", type=str, required=True, help="path like outputs/<exp>/<ckpt_id>")
    p_recon.add_argument("--spect229-dir", type=str, default="datasets/SPECT229")
    p_recon.add_argument("--orbit-file", type=str, default=None, help="override orbit.orb (default: osemreocnexe/orbit.orb)")
    ...
    def _cmd_gif_results(args: argparse.Namespace) -> None:
        from prime.pipeline.threeproj3recon import generate_gif_counts_from_existing
    ...
    p_gif.add_argument("--results-dir", type=str, required=True, help="path like outputs/<exp>/<ckpt_id>")
