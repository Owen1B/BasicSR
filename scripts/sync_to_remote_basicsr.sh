...
# - spect_ct/          (your local patient/report tooling & results)
...
if [[ "${DATASETS_MODE}" == "none" || "${DATASETS_MODE}" == "0" ]]; then
  ...
  echo "Exclude: spect_ct/ experiments/ datasets/ results/ wandb/ tb_logger/ __pycache__/ .git/ ..."
elif [[ "${DATASETS_MODE}" == "all" ]]; then
  echo "Exclude: spect_ct/ experiments/ results/ wandb/ tb_logger/ __pycache__/ .git/ ... (datasets/ INCLUDED: ALL)"
  ...
  echo "Exclude: spect_ct/ experiments/ results/ wandb/ tb_logger/ __pycache__/ .git/ ... (datasets/ INCLUDED: needed subset)"
else
