...
from prime.pipeline.io import load_dat_file
...
def load_mu_map_zyx(mu_path: Path) -> np.ndarray:
    return load_dat_file(mu_path, shape=(128, 128, 128), dtype=np.float32).astype(np.float32, copy=False)
...
def apply_view_alignment(
    ...
    *,
    ...
    """Apply deterministic alignment transforms on (V,H,W) volume."""
    y = np.asarray(x, dtype=np.float32)
    if bool(reverse_views):
        y = y[::-1]
    if int(view_shift) != 0:
        y = np.roll(y, shift=int(view_shift), axis=0)
    if bool(flip_y):
        y = np.flip(y, axis=1)
    if bool(flip_x):
        y = np.flip(y, axis=2)
...
def main() -> None:
    ...
    parser.add_argument("--align-reverse-views", action="store_true", help="mode=views: reverse view order before save")
    parser.add_argument("--align-view-shift", type=int, default=0, help="mode=views: circular shift on view axis")
    parser.add_argument("--align-flip-y", action="store_true", help="mode=views: flip height axis")
    parser.add_argument("--align-flip-x", action="store_true", help="mode=views: flip width axis")
    ...
    for pat in patients:
        ...
        if not mu_path.exists():
            ...
            if args.mode == "ap":
                ...
                li = apply_view_alignment(
                    li,
                    reverse_views=bool(args.align_reverse_views),
                    view_shift=int(args.align_view_shift),
                    flip_y=bool(args.align_flip_y),
                    flip_x=bool(args.align_flip_x),
                )
                mip = apply_view_alignment(
                    mip,
                    reverse_views=bool(args.align_reverse_views),
                    view_shift=int(args.align_view_shift),
                    flip_y=bool(args.align_flip_y),
        ...
        if args.mode == "ap":
            ...
            np.savez_compressed(
                ...
                li=li,
                mip=mip,
                start_angle_deg=float(args.start_angle_deg),
                angle_step_deg=float(args.angle_step_deg),
                align_reverse_views=bool(args.align_reverse_views),
                align_view_shift=int(args.align_view_shift),
                align_flip_y=bool(args.align_flip_y),
                align_flip_x=bool(args.align_flip_x),
            )
        done += 1
        if done % 25 == 0:
