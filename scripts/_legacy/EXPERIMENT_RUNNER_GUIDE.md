...
```bash
bash scripts/run_spect_selfsup_experiments.sh <tier> <mode>
```
...
```bash
bash scripts/run_spect_selfsup_experiments.sh tier1 all
```
**Note**: Noiser2Noise is excluded due to theoretical issues (compound Poisson distribution)

### Run a specific method
```bash
bash scripts/run_spect_selfsup_experiments.sh tier1 n2n          # Noise2Noise only
bash scripts/run_spect_selfsup_experiments.sh tier1 n2b          # Neighbor2Neighbor only
bash scripts/run_spect_selfsup_experiments.sh tier1 n2v          # Noise2Void only
bash scripts/run_spect_selfsup_experiments.sh tier1 s2s          # Self2Self only
```
...
```bash
bash scripts/run_spect_selfsup_experiments.sh tier2 all
```
...
```bash
bash scripts/run_spect_selfsup_experiments.sh tier2 norm
```
...
```bash
bash scripts/run_spect_selfsup_experiments.sh tier2 regularization
```
Configs: `wd_0`, `wd_1e5`, `wd_1e4`, `wd_5e4`, `wd_1e3`

#### Optimization / Learning Rate (7 configs)
```bash
bash scripts/run_spect_selfsup_experiments.sh tier2 optimization
```
Configs: `adam_1e4`, `adamw_1e4`, `adamw_5e5`, `adamw_5e4`, `etamin_1e7`, `etamin_1e6`, `etamin_1e4`

#### Data Strategy / Patch & Batch (7 configs)
```bash
bash scripts/run_spect_selfsup_experiments.sh tier2 data_strategy
```
...
```bash
bash scripts/run_spect_selfsup_experiments.sh tier2 network
```
Configs: `nb2`, `nb4`, `nb6`, `nc_32_64_128_256`, `nc_96_192_384_768`

#### Combined / Final Comparison (3 configs)
```bash
bash scripts/run_spect_selfsup_experiments.sh tier2 combined
```
...
```bash
bash scripts/run_spect_selfsup_experiments.sh tier3 n2v          # Noise2Void experiments
bash scripts/run_spect_selfsup_experiments.sh tier3 n2b          # Neighbor2Neighbor experiments
bash scripts/run_spect_selfsup_experiments.sh tier3 s2s          # Self2Self experiments
```
...
### Specify GPU
```bash
CUDA_VISIBLE_DEVICES=0 bash scripts/run_spect_selfsup_experiments.sh tier1 all
CUDA_VISIBLE_DEVICES=1 bash scripts/run_spect_selfsup_experiments.sh tier2 norm
```

### Run in background with logging
```bash
nohup bash scripts/run_spect_selfsup_experiments.sh tier2 all > tier2_all.log 2>&1 &
```
...
```bash
nohup bash scripts/run_spect_selfsup_experiments.sh tier2 all > tier2_full.log 2>&1 &
...
bash scripts/run_spect_selfsup_experiments.sh tier1 <method>
```
