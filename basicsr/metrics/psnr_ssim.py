...
def calculate_psnr_range(img, img2, crop_border, data_range, input_order='HWC', test_y_channel=False, **kwargs):
    """Calculate PSNR with a configurable data_range (peak value).

    This is useful for non-8bit data (e.g. medical counts) where values are not in [0,255].

    Args:
        img (ndarray): image.
        img2 (ndarray): reference image.
        crop_border (int): cropped pixels in each edge.
        data_range (float): peak-to-peak range (e.g. max_value in count domain).
        input_order (str): 'HWC' or 'CHW'. Default: 'HWC'.
        test_y_channel (bool): For RGB images only. Default: False.
    """
    assert img.shape == img2.shape, (f'Image shapes are different: {img.shape}, {img2.shape}.')
    if input_order not in ['HWC', 'CHW']:
        raise ValueError(f'Wrong input_order {input_order}. Supported input_orders are \"HWC\" and \"CHW\"')
...
def calculate_ssim_range(img, img2, crop_border, data_range, input_order='HWC', test_y_channel=False, **kwargs):
    """Calculate SSIM with configurable data_range.

    Args:
        img/img2 (ndarray): images.
        data_range (float): peak value used to define SSIM constants.
    """
    assert img.shape == img2.shape, (f'Image shapes are different: {img.shape}, {img2.shape}.')
    if input_order not in ['HWC', 'CHW']:
        raise ValueError(f'Wrong input_order {input_order}. Supported input_orders are \"HWC\" and \"CHW\"')
...
def _ssim_range(img, img2, data_range: float):
    """SSIM for one-channel images with configurable data_range."""
