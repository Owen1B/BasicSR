...
from prime.pipeline.inference import (
    denoise_views,
    load_basicsr_net,
...
from prime.pipeline.poisson import poisson_sample
from prime.pipeline.viz import compute_mip, create_subplot_with_colorbar, render_2x4_nogap_frame, rotate_volume_around_x
...
def generate_1x3_proj_gif(
    *,
    ...
    def _psnr_ssim_views_mean(pred: np.ndarray, ref: np.ndarray, data_range: float) -> tuple[float, float]:
        # Prefer BasicSR metrics (cv2-based) when available; fallback otherwise.
        try:
    ...
    lp_q_vgg = float("nan")
    try:
        from prime.pipeline.metrics import lpips_views_mean
...
def generate_2x4_gif(
    ...
    try:
        from prime.pipeline.metrics import lpips_recon_slices_mean
    ...
    for i in range(int(num_recon_frames)):
        ...
        from prime.pipeline.viz import Log1pNormalize
...
def process_patient_3proj3recon(
    *,
    ...
    for proj_file, recon_name, prj_dtype in recon_jobs:
        ...
        recon_out = ensure_osem_reconstruction(
            proj_file=proj_file,
            ...
            output_filename=recon_out.name,
            overwrite=not bool(skip_recon),
        )
        recon_paths[recon_name] = recon_out
