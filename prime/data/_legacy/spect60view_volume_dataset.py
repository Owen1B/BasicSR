...
from prime.data.poisson_thinning import (
    poisson_thinning_complement_split as n2n_binomial_split,
    poisson_thinning_same_dose_split as n2n_same_dose_two_splits_via_binomial,
)
    ...
    def _load_umap_aux(self, patient: str) -> dict[str, np.ndarray]:
        ...
        if not osp.exists(cache_path):
            ...
            if li.shape != (self.views, self.height, self.width):
                raise ValueError(
                    f"li shape mismatch in {cache_path}: got {li.shape}, "
                    f"expected {(self.views, self.height, self.width)}"
                )
            if mip.shape != (self.views, self.height, self.width):
                raise ValueError(
                    f"mip shape mismatch in {cache_path}: got {mip.shape}, "
                    f"expected {(self.views, self.height, self.width)}"
                )
    ...
    def __getitem__(self, index: int) -> Dict:
        ...
        if self.mode == "poisson":
            ...
            if self.n2n_k_choices is not None and self.n2n_split_mode == "same_dose":
                ...
                lq, gt = n2n_same_dose_two_splits_via_binomial(
                    vol, k=k, round_mode=self.n2n_round, random_swap=self.n2n_random_swap
                )
            else:
                lq, gt = n2n_binomial_split(
                    vol, p=self.n2n_p, round_mode=self.n2n_round, random_swap=self.n2n_random_swap
                )
        else:
