...
class Downsample3dCircularDepth(nn.Module):
    """Stride-(1,2,2) conv3d with explicit depth padding mode.

    Compared with plain Conv3d(..., padding=1), this keeps depth boundary behavior
    consistent with Conv3dCircularDepth: circular on D when enabled.
    """

    def __init__(self, in_ch: int, out_ch: int, bias: bool = True, circular_depth: bool = True) -> None:
        super().__init__()
        self.circular_depth = bool(circular_depth)
        self.conv = nn.Conv3d(
            in_ch,
            out_ch,
            kernel_size=(3, 3, 3),
            stride=(1, 2, 2),  # downsample only H/W
            padding=(0, 1, 1),  # depth padding handled manually
            bias=bias,
        )
...
def downsample_strideconv_3d(in_ch: int, out_ch: int, bias: bool = True, circular_depth: bool = True) -> nn.Module:
    return Downsample3dCircularDepth(in_ch, out_ch, bias=bias, circular_depth=circular_depth)
